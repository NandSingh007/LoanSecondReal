...
//     let { loanType } = user;
//     console.log(loanType, "khkdkfnskdnskfndskfnsnfsdnf,dsnf,d");
//     // loanType = "personal loan";
//     //     <select id="personalLoanDetails">
//     //     <option value="">Select option</option>
//     //     <option value="homeRenovation">Home Renovation Loan</option>
//     //     <option value="education">Travel Loan</option>
//     //     <option value="wedding">Wedding Loan</option>
//     //     <option value="wedding">Car Loan</option>
//     //     <option value="wedding">Medical Loan</option>
//     //     <option value="wedding">Mobile Loan</option>
...
exports.fetchCompanyProfiletwo = async (req, res) => {
  try {
    ...
    // Fetch company profile (assuming it is global or default for all users)
    const username = await Registration.findOne({ _id: userId });
    let name;
    if (username) {
    ...
    // Extract and normalize loanType from user data
    let { loanType } = user;
    ...
    // Fetch loan types based on the final loanType
    const loanTypes = await LoanTypeDatas.find({
...
exports.checkStatatcurity = async (req, res) => {
  const { userId } = req.params; // Extract userId from request parameters
  // console.log(userId, "shubham");
  try {
    // Find the document in SecurityData where the userId matches
    const securityDat = await SecurityData.findOne({ userId });
    const processingFeeDat = await processingFeeData.findOne({ userId });
    // console.log(securityData.Status, "securityData");
    // Check if securityData exists and return the Status
    if (securityDat && processingFeeDat) {
      res.status(200).json({
        StatusSec: securityDat.Status,
        StatusPro: processingFeeDat.Status
      });
    } else {
      res.status(200).json({ StatusSec: 0, StatusPro: 0 }); // Return false when no data is found
    }
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
exports.checkStatatSecurity = async (req, res) => {
  ...
  try {
    // Find the document in SecurityData where the userId matches
    const securityDat = await processingFeeData.findOne({ userId });
    // console.log(securityData.Status, "securityData");
    // Check if securityData exists and return the Status
    if (securityDat) {
      // console.log(securityDat.Status, "securityDat.Status");
      res.status(200).json({ Status: securityDat.Status });
    } else {
